...
Page({
    ...
    confirmModel: function (e) {
        var content_text = { 'refuse': '拒绝', 'confirm': '同意' };
        ...
        wx.showModal({
            ...
            success: function (res) {
                if (res.confirm) {
                    var url = '';
                    if (page.data.msg.type == 1) {
                        url = app.globalData.host + '/pair/lock/confirm/';
                    } else if (page.data.msg.type == 3) {
                        url = app.globalData.host + '/pair/query/confirm/';
                    }
                    wx.request({
                        url: url,
                        method: 'POST',
                        data: { 'msg_id': page.data.msg.id, 'confirm': confirm_number[e.target.id] },
                        header: { 'content-type': 'application/x-www-form-urlencoded', 'Cookie': app.globalData.cookie },
                        success: res => {
                            if (res.data.status == 'success') {
                                wx.navigateBack({
